...
def ParseToken(User, AuthorizationKey):
    return base64.urlsafe_b64encode(
        (User + ":" + AuthorizationKey).encode("ascii")
    ).decode("ascii")
...
def GetMatches(token):
    ...
    headers = {
        "Authorization": "Basic "
        + token,  # Decodes and gives you the Base64 in a string.
        "If-Modified-Since": "",
    }

    # Gets the most recent Event Match Results
    conn.request(
        "GET", "/v3.0/2025/matches/ARLI?tournamentLevel=Qualification", payload, headers
    )
...
def DownloadYoutubeVideoClip(url, folder, name, startTime, endTime):
    if platform.system() == "Linux":
        ...
        tmp = "C\\tmp"
        ydl_opts = {
            "extractor_DownloadYoutubeVideoClipargs": {
                "youtube": {
...
if args.URL and args.StartTime and args.Folder:
    Matches = GetMatches(args.Token or ParseToken(args.User, args.AuthorizationKey))

    StartTime = datetime.strptime(args.StartTime, "%M:%S.%f")
    StartTime = timedelta(minutes=StartTime.minute, seconds=StartTime.second)
    print(StartTime)

    ReferenceTime = Matches[0]["actualStartTime"] = datetime.fromisoformat(
        Matches[0]["actualStartTime"]
    )
    ...
    DownloadYoutubeVideoClip(
        ...
        (
            (datetime.fromisoformat(Matches[0]["postResultTime"]) - ReferenceTime)
            + StartTime
        ).total_seconds(),
    )
    ...
    for matchNumber in range(1, args.Matches):
        ...
        print(
            (datetime.fromisoformat(Match["actualStartTime"]) - ReferenceTime)
            + StartTime
        )
        DownloadYoutubeVideoClip(
            args.URL,
            ...
            (
                (datetime.fromisoformat(Match["postResultTime"]) - ReferenceTime)
                + StartTime
            ).total_seconds(),
        )
        """
